(ns brazilian-utils.inscricao-estadual.validators.df
  (:require [brazilian-utils.helpers :as helpers]))

;; Distrito Federal (DF): 13 digits
;; Format: 07XXXXXXXXX-XX (must start with 07, then 9 body digits + 2 check digits)
...
(defn is-valid? [ie-str]
  (let [ie (helpers/only-numbers ie-str)]
    (if (and (= 13 (count ie))
             (= "07" (subs ie 0 2)))  ; Must start with 07
